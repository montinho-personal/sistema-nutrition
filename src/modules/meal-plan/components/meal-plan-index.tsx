...
import { ArrowRightIcon, TargetIcon, UsersIcon, UtensilsIcon } from "lucide-react";
...
/**
 * Índice do Plano Alimentar: o cardápio exige a cadeia completa (diagnóstico
 * concluído + estratégia com peso). Sinaliza quem já está pronto.
 */
export function MealPlanIndex() {
  ...
  return (
    <>
      ...
      {ordered.length === 0 ? (
        <EmptyState
          icon={<UsersIcon />}
          title="Nenhum aluno para montar cardápio ainda"
          description="Cadastre um aluno, conclua o diagnóstico e gere a estratégia para montar o Plano Alimentar."
          action={
            <Button asChild>
        ...
        <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
          {ordered.map((student) => {
            const ready =
              completedIds.has(student.id) && withInput.has(student.id) && Boolean(student.mainGoal);
            return (
