...
export function buildAnamnesePortrait(answers: AnswerMap): PortraitGroup[] {
  const groups = [
    group(
      "Dia alimentar",
      [
        ["follows_diet", "Já segue dieta"],
        ["current_diet_detail", "Dieta atual (relato)"],
        ...
        ["meals_out_food", "Costuma pedir fora"],
        ["water_intake", "Água"],
        ["beverages", "Bebidas"],
    ...
    group(
      ...
      [
        ["trains", "Treina"],
        ["training_type", "Tipos de treino"],
        ["training_days_per_week", "Dias de treino/semana"],
        ["training_duration", "Duração do treino (min)"],
        ["training_split_detail", "Divisão dos treinos"],
        ["activity", "Rotina fora do treino"],
      ],
      answers,
    ),
    group(
      "Orçamento",
      [
        ["budget", "Orçamento de comida"],
        ["supplement_openness", "Abertura a suplementos"],
        ["supplement_budget", "Verba p/ suplementos"],
        ["shopping_frequency", "Compras"],
      ],
      answers,
    ),
    group(
