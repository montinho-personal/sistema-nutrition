...
begin
  ...
  select count(*) into n from public.profiles where id = test_user;
  ...
  -- Inserção --------------------------------------------------------------------
  insert into public.students (owner_id, full_name, sex, height_cm, main_goal, created_by)
  ...
  select updated_at into ts_before from public.students where id = test_student;
  perform set_config('app.change_reason', 'Ajuste de altura após nova medição', true);
  update public.students set height_cm = 179.0 where id = test_student;

  select version into v from public.students where id = test_student;
  ...
  -- Soft delete ---------------------------------------------------------------------
  update public.students set is_active = false where id = test_student;
  select count(*) into n from public.students where id = test_student;
  assert n = 1, 'TESTE 06 FALHOU: soft delete não deve remover a linha';
  update public.students set is_active = true where id = test_student;
  raise notice 'TESTE 06 OK — soft delete preserva o registro';

  -- Integridade referencial: FK inválida deve falhar -----------------------------------
  failed := false;
  begin
    insert into public.student_measurements (student_id, weight_kg)
  ...
  failed := false;
  begin
    insert into public.diagnosis_sessions (student_id, status, overall_confidence)
  ...
  insert into public.diagnosis_sessions (student_id, status, overall_confidence, created_by)
  values (test_student, 'in_progress', 72, test_user)
  returning id into test_session;

  insert into public.diagnosis_answers (session_id, block, question_key, question_text, answer)
  ...
  insert into public.diagnosis_scores (session_id, score_key, score)
  ...
  insert into public.diagnosis_hypotheses (session_id, hypothesis, confidence)
  values (test_session, 'Maior dificuldade será o período noturno', 88);
  insert into public.diagnosis_risks (session_id, risk_type, description, severity, mitigation)
  values (test_session, 'abandonment', 'Histórico de abandono em dietas restritivas',
          'high', 'Estratégia flexível com plano B');
  insert into public.diagnosis_opportunities (session_id, description, effort)
  ...
  select count(*) into n from public.strategy_versions
  ...
  perform set_config('app.change_reason', 'Ativação após validação do conselho', true);
  update public.strategies set status = 'active' where id = test_strategy;
  perform set_config('app.change_reason', '', true);

  select count(*) into n from public.strategy_versions where strategy_id = test_strategy;
  ...
  -- Cadeia nutricional ------------------------------------------------------------------------
  select id into test_food from public.foods limit 1;
  if test_food is null then
    insert into public.foods (name, energy_kcal, protein_g, carbs_g, fat_g,
                              source_id, source_code)
    select 'Arroz, integral, cozido', 124, 2.6, 25.8, 1.0, id, 'BRC0001'
    from public.food_sources where name = 'TBCA'
    returning id into test_food;
  end if;

  insert into public.macro_plans (strategy_id, plan_type, calories_kcal, protein_g, carbs_g, fat_g)
  values (test_strategy, 'standard', 2100, 160, 210, 62)
  returning id into test_plan;

  insert into public.meal_structures (macro_plan_id, position, name, objective)
  values (test_plan, 1, 'Café da manhã', 'Saciedade matinal')
  returning id into test_meal;

  insert into public.meal_items (meal_structure_id, food_id, quantity_grams, household_measure)
  ...
  failed := false;
  begin
    delete from public.diagnosis_sessions where id = test_session;
  exception when foreign_key_violation then
    failed := true;
  end;
  assert failed,
  ...
  declare
    orphan_session uuid;
  begin
    insert into public.diagnosis_sessions (student_id, status)
    ...
    delete from public.diagnosis_sessions where id = orphan_session;

    select count(*) into n from public.diagnosis_answers where session_id = orphan_session;
  ...
  insert into public.reports (student_id, strategy_id, title, content)
  ...
  select count(*) into n from public.report_versions where report_id = test_report;
  ...
  failed := false;
  begin
    insert into public.food_substitutions (food_id, substitute_food_id)
