...
comment on table public.diagnosis_sessions is
  'Sessão de Entrevista Estratégica (Documento 06). Cada nova entrevista gera nova sessão — histórico preservado.';
comment on column public.diagnosis_sessions.executive_summary is
  'Resumo Executivo gerado ao concluir (Documento 06 — Motor de Resumo).';
comment on column public.diagnosis_sessions.overall_confidence is
  'Confiança geral do diagnóstico 0–100 (Documento 03B — Grau de Confiança).';

create index idx_diagnosis_sessions_student on public.diagnosis_sessions (student_id);
create index idx_diagnosis_sessions_status on public.diagnosis_sessions (status);
create index idx_diagnosis_sessions_created_at on public.diagnosis_sessions (created_at);
...
comment on table public.diagnosis_answers is
  'Respostas da entrevista adaptativa, por bloco e pergunta (Documento 03A — 20 blocos).';
comment on column public.diagnosis_answers.block is
  'Bloco do diagnóstico — ex.: goals, routine, eating_behavior, psychology.';
comment on column public.diagnosis_answers.answer is
  'Resposta em formato flexível (texto, escala, múltipla escolha).';
comment on column public.diagnosis_answers.answer_state is
  'Estado da pergunta (Documento 07): respondida, em análise, confirmada, revisada.';

create index idx_diagnosis_answers_session on public.diagnosis_answers (session_id);
create index idx_diagnosis_answers_block on public.diagnosis_answers (block);
...
comment on table public.diagnosis_scores is
  'Scores do diagnóstico 0–100 (Documento 06): aderência, organização, motivação, risco...';
comment on column public.diagnosis_scores.score_key is
  'Identificador do score — ex.: adherence, organization, motivation, risk, satiety.';

create index idx_diagnosis_scores_session on public.diagnosis_scores (session_id);

-- diagnosis_hypotheses -----------------------------------------------------------------
create table public.diagnosis_hypotheses (
  id uuid primary key default gen_random_uuid(),
  session_id uuid not null references public.diagnosis_sessions (id) on delete cascade,
...
comment on table public.diagnosis_hypotheses is
  'Hipóteses com nível de confiança (Documento 03A — problema, justificativa, confiança, impacto, plano preventivo).';

create index idx_diagnosis_hypotheses_session on public.diagnosis_hypotheses (session_id);
create index idx_diagnosis_hypotheses_status on public.diagnosis_hypotheses (status);

-- diagnosis_risks -------------------------------------------------------------------------
create table public.diagnosis_risks (
  id uuid primary key default gen_random_uuid(),
  session_id uuid not null references public.diagnosis_sessions (id) on delete cascade,
...
comment on table public.diagnosis_risks is
  'Riscos do Motor de Risco (Documento 00): abandono, fome, perda muscular, psicológico, financeiro, rotina.';
comment on column public.diagnosis_risks.mitigation is
  'Como reduzir o risco — todo risco vem acompanhado de solução (Documento 02).';

create index idx_diagnosis_risks_session on public.diagnosis_risks (session_id);
create index idx_diagnosis_risks_type on public.diagnosis_risks (risk_type);

-- diagnosis_opportunities --------------------------------------------------------------------
create table public.diagnosis_opportunities (
  id uuid primary key default gen_random_uuid(),
  session_id uuid not null references public.diagnosis_sessions (id) on delete cascade,
  description text not null,
  expected_impact text,
...
comment on table public.diagnosis_opportunities is
  'Oportunidades do Motor de Oportunidades (Documento 00): pequenas mudanças de grande impacto.';
comment on column public.diagnosis_opportunities.priority is
  'Menor número = maior prioridade (Matriz impacto × dificuldade, Documento 03D).';

create index idx_diagnosis_opportunities_session on public.diagnosis_opportunities (session_id);
...
alter table public.diagnosis_hypotheses enable row level security;
alter table public.diagnosis_risks enable row level security;
alter table public.diagnosis_opportunities enable row level security;

create policy "diagnosis_sessions_all_authenticated" on public.diagnosis_sessions
  for all using (auth.uid() is not null) with check (auth.uid() is not null);
create policy "diagnosis_answers_all_authenticated" on public.diagnosis_answers
  for all using (auth.uid() is not null) with check (auth.uid() is not null);
create policy "diagnosis_scores_all_authenticated" on public.diagnosis_scores
  for all using (auth.uid() is not null) with check (auth.uid() is not null);
create policy "diagnosis_hypotheses_all_authenticated" on public.diagnosis_hypotheses
  for all using (auth.uid() is not null) with check (auth.uid() is not null);
create policy "diagnosis_risks_all_authenticated" on public.diagnosis_risks
  for all using (auth.uid() is not null) with check (auth.uid() is not null);
create policy "diagnosis_opportunities_all_authenticated" on public.diagnosis_opportunities
  for all using (auth.uid() is not null) with check (auth.uid() is not null);
