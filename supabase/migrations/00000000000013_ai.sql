...
comment on column public.ai_prompts.inputs is 'Descrição estruturada das entradas esperadas.';
comment on column public.ai_prompts.outputs is 'Descrição estruturada das saídas esperadas.';
...
create index idx_ai_prompt_versions_prompt on public.ai_prompt_versions (prompt_id);
...
comment on column public.ai_outputs.model is 'Identificador do modelo — arquitetura independente de provedor.';

create index idx_ai_outputs_prompt on public.ai_outputs (prompt_id);
create index idx_ai_outputs_student on public.ai_outputs (student_id);
create index idx_ai_outputs_created_at on public.ai_outputs (created_at);
...
comment on table public.ai_reasoning_logs is
  'Raciocínio passo a passo de cada execução — alimenta o AI Strategy Panel (Documento 09).';

create index idx_ai_reasoning_logs_output on public.ai_reasoning_logs (ai_output_id);

-- ai_recommendations ------------------------------------------------------------------------------
create table public.ai_recommendations (
  id uuid primary key default gen_random_uuid(),
  student_id uuid not null references public.students (id) on delete cascade,
  source_output_id uuid references public.ai_outputs (id),
...
comment on column public.ai_recommendations.status is
...
create policy "ai_prompts_all_authenticated" on public.ai_prompts
  for all using (auth.uid() is not null) with check (auth.uid() is not null);
create policy "ai_prompt_versions_all_authenticated" on public.ai_prompt_versions
  for all using (auth.uid() is not null) with check (auth.uid() is not null);
create policy "ai_outputs_all_authenticated" on public.ai_outputs
  for all using (auth.uid() is not null) with check (auth.uid() is not null);
create policy "ai_reasoning_logs_all_authenticated" on public.ai_reasoning_logs
  for all using (auth.uid() is not null) with check (auth.uid() is not null);
create policy "ai_recommendations_all_authenticated" on public.ai_recommendations
  for all using (auth.uid() is not null) with check (auth.uid() is not null);
